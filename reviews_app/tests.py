...
class ReviewsTests(APITestCase):
    ...
    def test_list_reviews_server_error_500(self):
        ...
        with patch('reviews_app.api.views.ReviewViewSet.get_queryset', side_effect=Exception('Database Error')):
            res = self.client.get("/api/reviews/")
    ...
    def test_create_review_server_error_500(self):
        self.client.force_authenticate(user=self.another_customer)
        self.client.raise_request_exception = False
        with patch('reviews_app.models.Review.objects.create', side_effect=Exception('Database Error')):
    ...
    def test_patch_review_server_error_500(self):
        self.client.force_authenticate(user=self.customer_user)
        self.client.raise_request_exception = False
        with patch('reviews_app.api.views.ReviewViewSet.get_object', side_effect=Exception('Database Error')):
