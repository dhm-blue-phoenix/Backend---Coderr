...
class OrdersTests(APITestCase):
    ...
    def test_list_orders_server_error_500(self):
        ...
        with patch('orders.api.views.OrderViewSet.get_queryset', side_effect=Exception('Database Error')):
            response = self.client.get("/api/orders/")
    ...
    def test_create_order_server_error_500(self):
        self.auth(self.customer_token)
        self.client.raise_request_exception = False
        with patch('orders.api.views.Order.objects.create', side_effect=Exception('Database Error')):
            response = self.client.post(
                "/api/orders/",
                {"offer_detail_id": self.offer_detail.id},
                format="json"
    ...
    def test_patch_order_server_error_500(self):
        self.auth(self.business_token)
        self.client.raise_request_exception = False
        with patch('orders.api.views.OrderViewSet.get_object', side_effect=Exception('Database Error')):
            response = self.client.patch(
                f"/api/orders/{self.order_id}/",
                {"status": "completed"},
                format="json"
    ...
    def test_order_count_server_error_500(self):
        self.auth(self.customer_token)
        self.client.raise_request_exception = False
        with patch('orders.api.views.Order.objects.filter', side_effect=Exception('Database Error')):
            response = self.client.get(f"/api/order-count/{self.business_user.id}/")
            self.assertEqual(response.status_code, status.HTTP_500_INTERNAL_SERVER_ERROR)
