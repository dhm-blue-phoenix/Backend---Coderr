...
class AuthenticationTests(APITestCase):
    ...
    def test_registration_server_error_500(self):
        ...
        with patch('authentication.models.User.objects.create_user', side_effect=Exception('Database Error')):
            response = self.client.post(
                self.registration_url, self.customer_payload, format="json"
            )
    ...
    def test_login_server_error_500(self):
        ...
        self.client.raise_request_exception = False
        with patch('authentication.models.User.objects.get', side_effect=Exception('Database Error')):
            response = self.client.post(self.login_url, login_payload, format="json")
            self.assertEqual(response.status_code, status.HTTP_500_INTERNAL_SERVER_ERROR)
